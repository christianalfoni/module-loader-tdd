(function () {
    ...
    var definedModules = [];
    var wrappedNodeRequire = null;
    var initializedModules = [];
    var isTest = null;
    var timeoutLimit = 100;

    var p = {
        overrideNodeRequire: function () {
            var require = module.require;
            module.require = function (name, calledFromLib) {
                if (!isNode && !calledFromLib) {
                    p.throw('You are not using the require function passed to the module, insert the require argument. You required ' + name);
                }
                return require.call(module, name);
            };
            wrappedNodeRequire = module.require;
        },
        ...
        nodeRequire: function (name, modules) {
            var module = wrappedNodeRequire(name, true);
        ...
        requireModules: function (path, callback) {

            if (!path) {
                p.throw('You have to define a path to where your modules are. Use "modules.path = __dirname" as relative path.');
            }

            var readdir = p.nodeRequire('recursive-readdir'),
                modules = [];

            readdir(path, function (err, files) {
                files.forEach(function (file) {
                    if (!file.match(/node_modules/) && file.match(/\.js$/)) {
                        modules.push(file);
                    }
                });
                modules.forEach(function (module) {
                    p.nodeRequire(module);
                });
                callback();
            });
        }
    ...
    var modules = {
        ...
        initialize: function (callback) {
            ...
            if (isNode) {
                p.overrideNodeRequire();
                p.requireModules(this.path, function () {
                    p.registerModules(definedModules, initializedModules);
