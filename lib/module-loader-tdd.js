    ...
    var p = {
        ...
        createResourceHandler: function (ignoreRegister) {
            ...
            var resourceCallbacks = {},
                createResourceCallback = function (type, callback) {
                    var func = function (event) {
                        var result = callback.apply(globalObject, event.detail.value);
                        event.detail.result = result || true; // Needs to indicate that it has been handled
                    };
                    resourceCallbacks[type] = func;
                    return func;
                };
            ...
            return {
                register: function (type, callback) {
                    ...
                    if (isNode) {
                        emitter.addListener('resource:' + type, createResourceCallback(type, callback));
                    } else {
                        globalObject.addEventListener('resource:' + type, createResourceCallback(type, callback));
                    }

                },
                fetch: function (type) {
                    var detail = {
                            value: Array.prototype.slice.call(arguments, 0).splice(1, arguments.length)
                        },
                        event = isNode ? {detail: detail} : new CustomEvent('resource:' + type, {
                            detail: detail
                        });

                    isNode ? emitter.emit('resource:' + type, event) : globalObject.dispatchEvent(event);
                ...
                remove: function (name) {
                    if (resourceCallbacks[name]) {
                        ...
                        if (isNode) {
                            emitter.removeListener('resource:' + name, resourceCallbacks[name]);
                        } else {
                            globalObject.removeEventListener('resource:' + name, resourceCallbacks[name]);
                        }
                ...
                teardown: function () {
                    for (var callback in resourceCallbacks) {
                        if (isNode) {
                            emitter.removeListener('resource:' + callback, resourceCallbacks[callback]);
                        } else {
                            globalObject.removeEventListener('resource:' + callback, resourceCallbacks[callback]);
                        }
